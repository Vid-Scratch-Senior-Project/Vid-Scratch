...
export default function PoisoningProcessor() {
    ...
    useEffect(() => {
        const unlisten = listen<{ stage: string; message: string; percent: number }>(
            "attack-progress",
            (event) => {
                const { stage, message, percent } = event.payload;
                setProgressMessage(message);
                if (typeof percent === 'number') setProgressPercent(percent);
                if (stage === "error") { setStatus("error"); setError(message); }
                if (stage === "done") setProgressPercent(100);
            }
        );
    ...
    useEffect(() => {
        ...
        if (attackResult.orig_top5 && attackResult.orig_top5_names) {
            setCleanPredictions(attackResult.orig_top5.map(([classId, conf], i) => ({
                classId, className: attackResult.orig_top5_names?.[i] || `class_${classId}`, confidence: conf,
            })));
        } else {
            setCleanPredictions([{ classId: attackResult.orig_pred, className: attackResult.orig_pred_name, confidence: attackResult.orig_confidence }]);
        }
        if (attackResult.adv_top5 && attackResult.adv_top5_names) {
            setPoisonedPredictions(attackResult.adv_top5.map(([classId, conf], i) => ({
                classId, className: attackResult.adv_top5_names?.[i] || `class_${classId}`, confidence: conf,
            })));
        } else {
            setPoisonedPredictions([{ classId: attackResult.verified_pred, className: attackResult.verified_pred_name, confidence: attackResult.adv_confidence }]);
        }
    }, [attackResult]);

    useEffect(() => {
        setCleanPredictions([]); setPoisonedPredictions([]); setPoisonedVideoUrl("");
        setAttackResult(null); setStatus('idle'); setError(""); setProgressMessage(""); setProgressPercent(0);
        if (showStats) {
            setShowStats(false);
            const w = getCurrentWindow();
            w.setResizable(true).then(() => w.setSize(new LogicalSize(BASE_WIDTH, WINDOW_HEIGHT)).then(() => w.center().then(() => w.setResizable(false)))).catch(() => {});
        }
    }, [videoUrl]);
    ...
    return (
        <div style={{ fontFamily: 'var(--font-poppins)', width: '100%', height: '100vh', overflow: 'hidden' }}>
            <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
                ...
                <div style={{
                    ...
                    <PoisoningProcessorOutput
                        videoUrl={videoUrl} intensity={intensity} quality={quality}
                        poisonedVideoUrl={poisonedVideoUrl} setPoisonedVideoUrl={setPoisonedVideoUrl}
                        status={status} setStatus={setStatus}
                        progressMessage={progressMessage} setProgressMessage={setProgressMessage}
                        progressPercent={progressPercent} setProgressPercent={setProgressPercent}
                        attackResult={attackResult} setAttackResult={setAttackResult}
                        error={error} setError={setError}
                        hasStats={hasStats} showStats={showStats} onToggleStats={toggleStats}
                    />
                </div>

                {/* Column 2: Video previews */}
                <div style={{
                    flex: 1, minWidth: 0, height: '100%', overflow: 'hidden',
                    borderRight: showStats ? '2px solid #000' : 'none',
                }}>
                    <PoisoningProcessorPreview videoUrl={videoUrl} poisonedVideoUrl={poisonedVideoUrl} status={status} />
                </div>
                ...
                {showStats && hasStats && (
                    <div style={{
                        ...
                        {/* Top: Clean */}
                        <div style={{
                            flex: 1, minHeight: 0, padding: '0.8rem',
                            display: 'flex', flexDirection: 'column',
                            borderBottom: '2px solid #000',
                        }}>
                            <PredictionPanel title="Clean - Top-5 Predictions" titleColor="#60a5fa" predictions={cleanPredictions} barColor="#3b82f6" />
                        </div>
                        {/* Bottom: Poisoned */}
                        <div style={{
                            flex: 1, minHeight: 0, padding: '0.8rem',
                            ...
                            {poisonedPredictions.length > 0 ? (
                                <PredictionPanel title="Poisoned - Top-5 Predictions" titleColor="#f87171" predictions={poisonedPredictions} barColor="#ef4444" />
                            ) : (
                                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--septenary-text-color)', fontSize: '0.8rem', backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: '8px' }}>
...
export function PredictionPanel({ title, titleColor, predictions, barColor }: {
    title: string; titleColor: string; predictions: PredictionEntry[]; barColor: string;
}) {
    ...
    return (
        <div style={{
            backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: '8px', padding: '12px 14px',
            display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0,
        }}>
            <div style={{ fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: titleColor, marginBottom: '3px', fontWeight: 600 }}>{title}</div>
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--primary-text-color)', marginBottom: '1px' }}>{topPred.className}</div>
            <div style={{ fontSize: '0.68rem', color: 'var(--septenary-text-color)', marginBottom: '8px' }}>Class #{topPred.classId} · Confidence: {(topPred.confidence * 100).toFixed(1)}%</div>
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'space-evenly' }}>
                {predictions.map((pred, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ color: 'var(--septenary-text-color)', minWidth: '16px', textAlign: 'right', fontSize: '0.65rem' }}>#{i + 1}</span>
                        <div style={{
                            ...
                            <span style={{
                                position: 'relative', zIndex: 1, lineHeight: '28px', paddingLeft: '8px',
                                color: i === 0 ? '#fff' : 'var(--septenary-text-color)',
                                fontWeight: i === 0 ? 600 : 400, fontSize: '0.75rem', whiteSpace: 'nowrap',
                            }}>
                                {pred.className}
                            </span>
                        </div>
                        <span style={{ color: 'var(--septenary-text-color)', minWidth: '42px', textAlign: 'right', fontSize: '0.7rem' }}>
